...
+ 由于客观原因：
  ...
  + 游戏有时会发生漏判现象。这会导致相同的触摸事件序列在应用于同一谱面时，所得分数并不相同。可能需要多尝试几次才能达到<span style="color: yellow">φ</span>的成绩。
...
### 由于当前`触摸事件规划算法`的缺陷，下面的曲目并不能拿到<span style="color: yellow">φ</span>
